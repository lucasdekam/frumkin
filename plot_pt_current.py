...
potentials_v_rhe = np.linspace(-0.7, -0.4, 100)
...
for i, conc in enumerate(conc_list):
    ...
    current_conc[i, :] = kinetics.edl_transport_limited_current(
        model,
...
for i, gamma in enumerate(P.GAMMA_LIST):
    ...
    current_gamma[i, :] = kinetics.edl_transport_limited_current(
        model,
        potentials_v_she + C.PT_PZC_SHE_V,
        pzc_she=C.PT_PZC_SHE_V,
        p_h=P.DEFAULT_P_H,
...
for i, p_h in enumerate(ph_list):
    ...
    current_ph[i, :] = kinetics.edl_transport_limited_current(
        model,
