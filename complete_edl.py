...
class Aqueous:
    ...
    def potential_sequential_solve(
        ...
        print(
            f"Gold: sweep from {potential[0]:.2f}V to {potential[-1]:.2f}V. "
    ...
    def potential_sweep(
        ...
        sweep_df["phi2"] = sweep_df["phi0"] - sweep_df["efield"] * C.D_ADSORBATE_LAYER
    ...
    def spatial_profiles(
        ...
        profiles = self.potential_sequential_solve(
            self.ode_lambda, np.arange(0, phi0, sign * 0.01), tol=tol, p_h=p_h
        )
        return profiles[-1]
    ...
    def insulator_sequential_solve(
        ...
        print(
            f"Insulator: sweep from pH {ph_range[0]:.2f} to {ph_range[-1]:.2f}."
            + f"Max. rel. residual: {np.max(max_res):.3e}."
        )
        return profile_list
    ...
    def insulator_spatial_profiles(self, p_h: float, tol: float = 1e-3) -> pd.DataFrame:
        ...
        profiles = self.insulator_sequential_solve(
            np.arange(ph_pzc, p_h + step, step), tol
        )
        return profiles[-1]
