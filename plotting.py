...
def plot_solutions(
    sol_list: list[pd.DataFrame], xmin: float, xmax: float, suppress_names=False
):
    """
    ...
    for i, sol in enumerate(sol_list):
        ...
        ax[0, 1] = plot_efield(ax[0, 1], sol["x"], sol["efield"], color=colors[i])
        ax[0, 2] = plot_permittivity(ax[0, 2], sol.x, sol.eps, color=colors[i])
        ax[1, 0] = plot_concentrations(
            ax[1, 0], "Cations", sol["x"], sol["cations"], color=colors[i]
        )
        if "hydroxide" in sol.columns:
            ax[1, 1] = plot_concentrations(
                ax[1, 1],
                "Anions",
                sol["x"],
            ...
            ax[1, 1] = plot_concentrations(
                ax[1, 1], "Anions", sol["x"], sol["anions"], color=colors[i]
            )
        if "solvent" in sol.columns:
            ax[1, 2] = plot_concentrations(
                ax[1, 2], "Solvent", sol["x"], sol["solvent"], color=colors[i]
            )
