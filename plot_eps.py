...
ax1.plot(sol_lpb["x"], sol_lpb["eps"], "k:", label="LPB")
ax1.plot(sol_gon["x"], sol_gon["eps"], "k", label="GI")
ax1.set_xlim([0, 5])
...
for i, gamma in enumerate(gamma_range):
    ax3.plot(gi_gamm[i]["x"], gi_gamm[i]["eps"], color=colors2[i], label=f"{gamma:.0f}")
ax3.legend(frameon=False, title=r"$\gamma_+$")

for i, phi0_v in enumerate(phi0_v_range):
    ax4.plot(
        gi_phi0[i]["x"], gi_phi0[i]["eps"], color=colors3[i], label=f"{phi0_v:.1f}"
    )
...
for label, axis in zip(labels, fig.axes):
    ...
    axis.set_ylim([0, 80])
    axis.set_xlim([0, 2])
    axis.set_ylabel(r"$\varepsilon/\varepsilon_0$")
    axis.set_xlabel(r"$x$ / nm")

plt.tight_layout()
plt.savefig("figures/res-eps-comparison.pdf")
plt.show()
