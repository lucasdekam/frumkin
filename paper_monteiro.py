...
df_au_li = pd.read_parquet("data/au_li.parquet")
df_au_li["name"] = [
    r"Li$^+$ pH 11" if value == 11 else r"Li$^+$ pH 13" for value in df_au_li["pH"]
]
df_au_li["color"] = "black"
df_au_li["fillstyle"] = ["full" if value == 13 else "none" for value in df_au_li["pH"]]

df_au_k = pd.read_parquet("data/au_k.parquet")
df_au_k["name"] = [
    r"K$^+$ pH 11" if value == 11 else r"K$^+$ pH 13" for value in df_au_li["pH"]
]
df_au_k["color"] = "red"
df_au_k["fillstyle"] = ["full" if value == 13 else "none" for value in df_au_k["pH"]]
...
final_df_1 = pd.concat([df_au_li_1, df_au_k_1])
...
final_df_2 = pd.concat([df_au_li_2, df_au_k_2])
...
x = C.E_0 * C.BETA * (df_au_li_1["phi0"] - df_au_li_1["phi_rp"])
y = df_au_li_1["log j"]
p1 = np.polyfit(x, y, deg=1)
print(p1)

x = C.E_0 * C.BETA * (df_au_li_2["phi0"] - df_au_li_2["phi_rp"])
y = df_au_li_2["log j"]
...
ax_au1.set_xlim([-1.5, -1.0])
ax_au2.set_xlim([-1.1, -0.6])
ax_au2.set_xticks([-1.1, -1.0, -0.9, -0.8, -0.7, -0.6])
...
plt.tight_layout()
plt.savefig("figures/res-monteiro.pdf")
