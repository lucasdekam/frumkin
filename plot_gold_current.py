...
potentials_v_rhe_catsize = np.linspace(
    -0.7, C.AU_PZC_SHE_V + 59e-3 * DEFAULT_P_H_CATSIZE, NUM_PTS
)
potentials_v_she_catsize = potentials_v_rhe_catsize - 59e-3 * DEFAULT_P_H_CATSIZE

potentials_v_rhe_conc = np.linspace(
    -0.7, C.AU_PZC_SHE_V + 59e-3 * DEFAULT_P_H_CONCENT, NUM_PTS
)
potentials_v_she_conc = potentials_v_rhe_conc - 59e-3 * DEFAULT_P_H_CONCENT
...
for i, conc in enumerate(conc_list):
    ...
    current_conc_frumkin[i, :] = kinetics.frumkin_corrected_current(
        model,
        potentials_v_she_conc,
        pzc_she=C.AU_PZC_SHE_V,
        p_h=DEFAULT_P_H_CONCENT,
        deltag=DELTAG,
    )
...
for i, gamma in enumerate(P.GAMMA_LIST):
    ...
    current_gamma_frumkin[i, :] = kinetics.frumkin_corrected_current(
        model,
        potentials_v_she_catsize,
        pzc_she=C.AU_PZC_SHE_V,
        p_h=DEFAULT_P_H_CATSIZE,
        deltag=DELTAG,
    )
...
for i, p_h in enumerate(ph_list):
    ...
    current_ph_frumkin[i, :] = kinetics.frumkin_corrected_current(
        model,
        potentials_v_rhe_catsize - 59e-3 * p_h,
        pzc_she=C.AU_PZC_SHE_V,
        p_h=p_h,
        deltag=DELTAG,
    )
