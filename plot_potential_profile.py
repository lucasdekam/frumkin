...
ax1.plot(sol_gcs["x"], sol_gcs["phi"], "k--", label="GCS")
ax1.plot(sol_lpb["x"], sol_lpb["phi"], "k:", label="LPB")
ax1.plot(sol_bik["x"], sol_bik["phi"], "k-.", label="Bikerman")
ax1.plot(sol_gon["x"], sol_gon["phi"], "k", label="GI")
ax1.set_xlim([0, 5])
...
for i, gamma in enumerate(gamma_range):
    ax3.plot(gi_gamm[i]["x"], gi_gamm[i]["phi"], color=colors2[i], label=f"{gamma:.0f}")
ax3.legend(frameon=False, title=r"$\gamma_+$")

for i, phi0_v in enumerate(phi0_v_range):
    ax4.plot(
        gi_phi0[i]["x"], gi_phi0[i]["phi"], color=colors3[i], label=f"{phi0_v:.1f}"
    )
...
for label, axis in zip(labels, fig.axes):
    ...
    axis.set_ylim([-1, 0.01])
    axis.set_xlim([0, 1])
    axis.set_ylabel(r"$\phi$ / V")
    axis.set_xlabel(r"$x$ / nm")
ax1.set_xlim([0, 5])
ax4.set_ylim([-2, 0])

plt.tight_layout()
plt.savefig("figures/res-phi-comparison.pdf")
plt.show()
